...
router.post('/google/login', (req,res,next)=>{
    ...
    .then(result=>{
        ...
        console.log(email);
        // if(email!="manik.aiotize@gmail.com"){ return res.json({message: "You are not admin"}).status(500); }
        if(email_verified){
...
router.post('/login', (req, res, next)=>{
    // if(req.body.email !="manik.aiotize@gmail.com"){ return res.json({message: "You are not admin"}).status(500); }
    User.find({ email: req.body.email })
    .exec()
    .then(user =>{
        if(user.length<1){
